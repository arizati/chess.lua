...
describe("Perft #perft", function()
    ...
    forEach(perfts, function(perft)
        local chess = Chess()
        chess.load(perft.fen)
        it(perft.fen, function()
            local nodes = chess.perft(perft.depth)
            assert.are.equals(perft.nodes, nodes)
        end)

    end)
end)

describe("Single Square Move Generation #movegen", function()
    ...
    local positions = {
        ...
        { fen = '8/7K/8/8/1R6/k7/1R1p4/8 b - - 0 1',
          ...
          moves = { { color = 'b', from = 'd2', to = 'd1', flags = 'np', piece = 'p', promotion = 'q', san = 'd1=Q' },
    ...
    forEach(positions, function(position)
        ...
        it(position.fen .. ' ' .. position.square, function()
            local moves = chess.moves({ square = position.square, verbose = position.verbose })
            assert.are.same(position.moves, moves)
        end)
    end)
...
describe("Stalemate #stale", function()
    local stalemates = {
...
describe("Get/Put/Remove #get #put #remove", function()
    ...
    forEach(positions, function(position)
        it("position should pass - " .. tostring(position.should_pass), function()
            -- places the pieces
            for square, piece in pairs(position.pieces) do
                passed = passed and chess.put(piece, square)
            end
            ...
            for _, square in ipairs(chess.SQUARES) do
                ...
                if pp == nil then
                    ...
                    local piece = chess.get(square)
                    if not (piece and piece.type == pp.type and piece.color == pp.color) then
                        passed = false
                        break
                    end
                end
            ...
            if passed then
                ...
                for _, square in ipairs(chess.SQUARES) do
                    ...
                    if piece and (pp.type ~= piece.type or pp.color ~= piece.color) then
                        passed = false
                        break
                    end
                end
            ...
            passed = passed and (chess.fen() == '8/8/8/8/8/8/8/8 w - - 0 1')

            -- some tests should fail, so make sure we're supposed to pass/fail each
            -- test
            assert.are.equals(position.should_pass, passed)
        end)
    end)
...
describe("Load PGN #loadpgn", function()
    ...
    forEach(tests, function(t, i)
        forEach(newline_chars, function(newline, j)
            it(i .. string.char(96 + j), function()
                local sloppy = t.sloppy or false
                local result = chess.load_pgn(table.concat(t.pgn, newline), { sloppy = sloppy, newline_char = newline })
                local should_pass = t.expect
                ...
                if should_pass then
                    ...
                    -- (instead of the reconstructed PGN)
                    if t.fen then
                        assert.are.equals(t.fen, chess.fen())
                    else
                        assert.are(table.concat(t.pgn, newline), chess.pgn({ max_width = 65, newline_char = newline }))
                    end

                else
                    -- this test should fail, so make sure it does
                    assert.is.not_true(result)
                end
            end)
    ...
    it('dirty pgn', function()
        ...
        local result = chess.load_pgn(pgn, { newline_char = '\r?\n' })
        assert.is_true(result)

        assert.is_true(chess.load_pgn(pgn))
        assert.is.falsy(chess.pgn():find('^%[%['))
    end)
...
describe("Make Move #mkmove", function()
    ...
    forEach(positions, function(position)
        local chess = Chess()
        chess.load(position.fen)
        it(string.format('%s (%s %s)', position.fen, position.move, tostring(position.legal)), function()
            local sloppy = position.sloppy or false
            local result = chess.move(position.move, { sloppy = sloppy })
            if position.legal then
                assert.truthy(result)
                assert.are.equals(position.next, chess.fen())
                assert.are.equals(position.captured, result.captured)
            else
                assert.falsy(result)
            end
        end)
...
describe("Validate FEN #vfen", function()
    ...
    forEach(positions, function(position)
        ...
        it(string.format('%s (valid: %s)', position.fen, tostring(position.error_number == 0)), function()
            local result = chess.validate_fen(position.fen)
            assert.are.equals(position.error_number, result.error_number)
        end)
...
describe("History #history", function()
    ...
    forEach(tests, function(t, i)
        ...
        it(i, function()
            ...
            local history = chess.history({ verbose = t.verbose })

            assert.are.equals(t.fen, chess.fen())
            assert.are.equals(#t.moves, #history)
            assert.are.same(t.moves, history)
...
describe('Board Tests #board', function()
    local null = false
    local tests = {
        { fen = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1',
...
describe('Regression Tests #reg', function()
    it('Github Issue #32 - castling flag reappearing', function()
        local chess = Chess('b3k2r/5p2/4p3/1p5p/6p1/2PR2P1/BP3qNP/6QK b k - 2 28')
        chess.move({ from = 'a8', to = 'g2' })
        assert.are.equals('4k2r/5p2/4p3/1p5p/6p1/2PR2P1/BP3qbP/6QK w k - 0 29', chess.fen())
    end)

    it('Github Issue #58 - placing more than one king', function()
        local chess = Chess('N3k3/8/8/8/8/8/5b2/4K3 w - - 0 1')
        assert.is_false(chess.put({ type = 'k', color = 'w' }, 'a1'))
        chess.put({ type = 'q', color = 'w' }, 'a1')
        chess.remove('a1')
        assert.are.equals('Kd2 Ke2 Kxf2 Kf1 Kd1', table.concat(chess.moves(), ' '))
    end)

    it('Github Issue #85 (white) - SetUp and FEN should be accepted in load_pgn', function()
        local chess = Chess()
        local pgn = { '[SetUp "1"]', '[FEN "7k/5K2/4R3/8/8/8/8/8 w KQkq - 0 1"]', "", '1. Rh6#' }
        local result = chess.load_pgn(table.concat(pgn, '\n'))
        assert.truthy(result)
        assert.are.equals('7k/5K2/7R/8/8/8/8/8 b KQkq - 1 1', chess.fen())
    end)

    it('Github Issue #85 (black) - SetUp and FEN should be accepted in load_pgn', function()
        local chess = Chess()
        local pgn = { '[SetUp "1"]', '[FEN "r4r1k/1p4b1/3p3p/5qp1/1RP5/6P1/3NP3/2Q2RKB b KQkq - 0 1"]', "", '1. ... Qc5+' }
        local result = chess.load_pgn(table.concat(pgn, '\n'))
        assert.truthy(result)
        assert.are.equals('r4r1k/1p4b1/3p3p/2q3p1/1RP5/6P1/3NP3/2Q2RKB w KQkq - 1 2', chess.fen())
    end)
    ...
    it('Github Issue #98 (white) - Wrong movement number after setting a position via FEN', function()
        ...
        chess.move('f7')
        local result = chess.pgn()
        assert.truthy(result:find('(45%. f7)$'))
    end)
    ...
    it('Github Issue #98 (black) - Wrong movement number after setting a position via FEN', function()
        ...
        chess.move('Rf1+')
        local result = chess.pgn()
        assert.truthy(result:find('(45%. %.%.%. Rf1%+)$'))
    end)
